export def zll [
  layout?: string
  session?: string
  --default (-d)
] {
  let layouts = (
    ls ~/.config/zellij/layouts/
    | get name
    | each { $in | path basename  }
    | where { $in | str ends-with '.kdl' }
    | each { $in | str replace '.kdl' '' }
  )

  let layout = (
    if $default { "default" }
    else if ($layout != null) { $layout }
    else {
      $layouts | str join "\n" | fzf
    }
  )

  if ($session == null) {
    zellij --new-session-with-layout $layout
  } else {
    zellij --new-session-with-layout $layout --session $session
  }

}

export def zl [
  session?: string
] {
  if $session != null {
    zll "test" $session
    return
  }

  let basename = (pwd | path basename)
  let t = $basename in (zellij list-sessions -ns | lines)
  
  if not $t {
    zll "test" $basename
    return
  }

  let exited = (zellij list-sessions -n | lines | where { $in | (split words | $in.0 == $basename)} | first | str contains "EXITED")
  
  if $exited {
    zellij kill-session $basename --force
    zll "test" $session
  } else {
    zellij attach $basename
  } 
}

export def zlc [
  session?: string
] {
  let basename = (pwd | path basename)
  let t = $basename in (zellij list-sessions -ns | lines )

  zll "coding" $session
}

export def zls [] {
  zellij list-sessions
}

export def zla [
  session?: string
] {
  let found_session = if ($session != null and ($session | is-not-empty) ) {
    zellij list-sessions --no-formatting --short | lines | where { $in | str contains $session } 
  }

  let len = ($found_session | length)

  if $len == 1 {
      zellij attach ($found_session | first)
      return null
  } 

  let session = if ($session != null) { $session } else {
    zellij list-sessions --no-formatting | fzf | cut -d' ' -f1
  }

  if ($session | str length) == 0 {
    print "Canceled"
    return null
  }
  
  zellij attach $session
}

export def zld [
  session?: string
  --force (-f)
] {
  let session = if ($session != null) { $session } else {
    zellij list-sessions --no-formatting | fzf | cut -d' ' -f1
  }

  if ($session | str length) == 0 {
    print "Canceled"
    return null
  }

  if ($force) {
    zellij delete-session $session --force
  } else {
    zellij delete-session $session
  }
}

export def zlk [
  session?: string
  --force (-f)
] {
  let session = if ($session != null) { $session } else {
    zellij list-sessions --no-formatting | fzf | cut -d' ' -f1
  }

  if ($session | str length) == 0 {
    print "Canceled"
    return null
  }

  if ($force) {
    zellij kill-session $session --force
  } else {
    zellij kill-session $session
  }
}

export def zlda [
  --force (-f)
] {
  if ($force) {
    zellij delete-all-sessions --force
  } else {
    zellij delete-all-sessions
  }
}

export def zlka [
  --force (-f)
] {
  if ($force) {
    zellij kill-all-sessions --force
  } else {
    zellij kill-all-sessions
  }
}
