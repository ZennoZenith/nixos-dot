...
__wezterm_set_user_var() {
  if command type -P base64 &>/dev/null ; then
    if [[ -z "${TMUX-}" ]] ; then
...
__wezterm_osc7() {
  if command type -P wezterm &>/dev/null ; then
...
if [[ -z "${WEZTERM_HOSTNAME}" ]]; then
  ...
  elif command type -P hostname &>/dev/null; then
    __wezterm_set_user_var "WEZTERM_HOST" "$(hostname)"
  elif command type -P hostnamectl &>/dev/null; then
